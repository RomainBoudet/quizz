...
// un score est lié a un seul user
Score.belongsTo(User, {
    foreignKey: "user_id",
    as:"user"
})

// un user peut être lié a 0 ou plusieurs scores
User.hasMany(Score, {
    foreignKey: "user_id",
    as: "scores"
});
...
// Scores <> Quiz, via la table de liaison
// "Un Quiz possède plusieurs scores"
Quiz.belongsToMany(Score, {
    as: "scoress", // alias de l'association 
    through: 'quiz_has_scores', // "via la table de liaison qui s'appelle ..."
    foreignKey: 'quiz_id', // le nom de la clef de Quiz dans la table de liaison
    otherKey: 'score_id', // le nom de la clef de "l'autre" (donc Tag)
});
// Et un score peut faire référence a plusieur Scores !
Score.belongsToMany(Quiz, {
    as: "quizzes",
    through: 'quiz_has_scores',
    foreignKey: 'score_id',
    otherKey: 'quiz_id',
});
...
module.exports = { Answer, Level, Question, Quiz, Tag, User, Score };
